...
export async function POST(request: NextRequest) {
  try {
    ...
    let response = await fetch(`${getFunctionsBaseUrl()}/getGlobalStatsHttp`, {
      method: 'POST',
      headers: {
    ...
    if (response.status === 404) {
      response = await fetch(`${getFunctionsBaseUrl()}/getGlobalStats`, {
        method: 'POST',
        headers: {
          Authorization: authHeader,
          'Content-Type': 'application/json',
    ...
    console.error('Error proxying getGlobalStatsHttp (admin fallback route):', error)
