...
export function LandingHeader({ isAuthenticated }: { isAuthenticated: boolean }) {
  ...
  const appTarget = profile ? getAccessTargetFromProfile(profile) : 'dashboard'
  ...
  const goToDashboard = () => {
    ...
    window.location.href = getAppHomeUrl(window.location, appTarget)
  }
      ...
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
        <Link href="/" className="flex items-center gap-3 group">
          ...
          <div className="hidden sm:block">
            ...
            <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground mt-0.5">Plattform fuer Schulen und Jahrgaenge</p>
          </div>
        </Link>

        <nav className="hidden lg:flex items-center gap-8">
          <Link href="/#features" className="text-sm font-medium text-muted-foreground hover:text-foreground transition-colors">Funktionen</Link>
        ...
        <div className="flex items-center gap-3">
          ...
          {isAuthenticated ? (
            <Button onClick={goToDashboard} size="sm" className="h-9 px-4 rounded-lg font-bold">
              {appTarget === 'tcg' ? 'Karten' : 'Dashboard'}
