...
const DiceRoller = forwardRef<DiceRollerRef, {}>((props, ref) => {
  ...
  return (
    <div className="perspective-1000 w-32 h-32 mx-auto">
      ...
      >
        {/* Face 1: Front */}
        <div className={faceStyle} style={{ transform: "rotateY(0deg) translateZ(64px)" }}>
          <DiceDots count={1} />
        </div>
        {/* Face 2: Back */}
        <div className={faceStyle} style={{ transform: "rotateY(180deg) translateZ(64px)" }}>
          <DiceDots count={2} />
        </div>
        {/* Face 3: Right */}
        <div className={faceStyle} style={{ transform: "rotateY(90deg) translateZ(64px)" }}>
          <DiceDots count={3} />
        </div>
        {/* Face 4: Left */}
        <div className={faceStyle} style={{ transform: "rotateY(-90deg) translateZ(64px)" }}>
        ...
        {/* Face 5: Top */}
        <div className={faceStyle} style={{ transform: "rotateX(90deg) translateZ(64px)" }}>
          <DiceDots count={5} />
        </div>
        {/* Face 6: Bottom */}
        <div className={faceStyle} style={{ transform: "rotateX(-90deg) translateZ(64px)" }}>
...
const DiceDots = ({ count }: { count: number }) => {
  ...
  return (
    <div className="grid grid-cols-3 grid-rows-3 w-20 h-20 p-2">
      {dotPositions[count as keyof typeof dotPositions].map((pos, i) => (
        <div key={i} className={`w-4 h-4 bg-black rounded-full self-center justify-self-center ${getDotClass(pos)}`} />
      ))}
    </div>
  );
};
