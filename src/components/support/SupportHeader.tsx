...
import { searchHelpFaqs, HelpFaqItem } from '@/lib/helpFaqs'
...
export function SupportHeader() {
  ...
  useEffect(() => {
    if (search.trim().length > 2) {
      const found = searchHelpFaqs(search, 5)
      setResults(found)
    } else {
      setResults([])
    }
      ...
      <div className="container mx-auto flex h-16 items-center justify-between px-4">
          ...
          {isSearchOpen && search.length > 2 && (
            <div className="absolute top-full left-0 w-full mt-2 bg-popover border rounded-xl shadow-2xl p-2 animate-in fade-in zoom-in-95 duration-200">
              <div className="flex items-center justify-between px-2 py-1 mb-1 border-b">
                <span className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Suchergebnisse</span>
        ...
        <div className="flex items-center gap-2">
          <Button variant="ghost" size="sm" className="hidden sm:flex" asChild>
            <a href={getMainBaseUrl()}>Zurück zum Planer</a>
          </Button>
          <Button size="sm" className="gap-2 rounded-full" onClick={() => router.push('/beschwerden')}>
            <HelpCircle className="h-4 w-4" />
            <span className="hidden xs:inline">Hilfe anfordern</span>
          </Button>
        </div>
      </div>
    </header>
